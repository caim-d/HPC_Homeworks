...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  long jb{BLOCK_SIZE}; // blocks over j loop (up to n)
  long pb{BLOCK_SIZE}; // blocks over p loop (up to k)
  for (long jj = 0; jj < n; jj+=jb) {
    for (long pp = 0; pp < k; pp+=pb) {
      for (long j = jj; j < jj+jb; j++) {
        for (long p = pp; p < pp+pb; p++) {
          for (long i = 0; i < m; i++) {
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double time = t.toc();
    double flops = 2.0*m*n*k*NREPEATS/1e9/time; // TODO: calculate from m, n, k, NREPEATS, time
    double bandwidth = 2*m*n*k *NREPEATS * sizeof(double)/1e9/time; // TODO: calculate from m, n, k, NREPEATS, time
